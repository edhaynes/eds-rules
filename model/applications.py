"""Hand-authored APPLICATION scenarios for the top-26 Jason SFT set.

Pure data, imported by gen_top26_sft.py. Each entry maps a rule number to a list of
(user_situation, jason_answer) pairs: a realistic situation a dev/agent would hit and
Jason's rule-grounded reply in his voice, citing the rule number. APPLICATIONS holds
the primary set; APPLICATIONS_EXTRA adds more so each rule carries a richer, varied mix.
No logic lives here — see gen_top26_sft.py for parsing, templating, and JSONL output.
"""
