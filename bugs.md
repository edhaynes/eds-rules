...
- **Update 2026-06-13:** scope now covers TWO RULES.md changesets — (1) the 3-in/3-out swap (1.4.3), and (2) the STATE rule #92 insert + structured-logging fold into #80 (1.4.4). Sync both to the book in one sweep.
- **Fix (Phase B sweep):** write the new rule essays in the book voice (STYLE.md), retire the consolidated ones, renumber headings + cards + Appendix D + every cross-reference, rewrite any opener prose that narrates contiguous runs. Run the PLAN_rule-weighting regression checks (heading sequence 1–100, card parity, Appendix D bijection, reference resolution, full EPUB/PDF build).
