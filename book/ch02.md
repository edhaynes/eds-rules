...
There is also an order of operations to design here, and it never varies: **requirements doc, then API, then tests, then implementation — frontend and backend in parallel.** First a requirements document says what the thing must do; one page is usually enough, and every gap in it will faithfully become a gap in the product, because the machine builds what you specified, not what you meant. Then the API contract: the seams, frozen before any implementation exists. Then the tests, written against that contract, pinning the behavior while there is still nothing to be precious about. Only then implementation — and because the contract is frozen and the tests are already waiting, frontend and backend proceed *in parallel*, by different people or different agents, without drifting apart. The contract is the agreement, the tests are its enforcement, and the implementations are interchangeable labor. This is also exactly what makes the crew's parallel sprints safe: two agents cannot collide on opposite sides of a frozen seam. Chapter 4 owns the testing half of this discipline — contract first, code second; this chapter owns the design half: deciding the seams well enough to freeze them.

```mermaid
flowchart LR
    REQ["Requirements doc<br/>what it must do"] --> API["API contract<br/>the seams, frozen"]
    API --> TESTS["Tests<br/>against the contract"]
    TESTS --> FE["Frontend"]
    TESTS --> BE["Backend"]
    FE --> SHIP["Integration<br/>suite green, ship"]
    BE --> SHIP
```
