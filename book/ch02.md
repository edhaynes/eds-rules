...
Twenty rules follow. The first ten put the changeable decisions in config. The second ten put the structural ones behind boundaries.
...
So: one config object, built once at startup, passed to everything that needs it (dependency injection, Rule 33). And one precedence order, increasing from broad to specific:
...
And validation means more than presence-checking. Parse the port as an integer and range-check it. Verify the URL scheme. Confirm the named backend is one you actually support. Fail fast, fail loud, fail *specific* — this is Rule 9 from Chapter 1's hard rules, applied to the config layer where it earns the most.
...
That's configuration: ten rules about pulling decisions out of the source and giving them names so they can change without a deploy. The next ten govern the decisions you can't pull out — who owns what, who talks to whom, and how big anything is allowed to get. Config decides the values; architecture decides the structure that consumes them, and the two meet at the constructor, where the config object and the collaborators it selects walk in through the same front door.
