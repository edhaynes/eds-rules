...
> - Some rules are never broken — scan before every boundary, never hardcode secrets, distrust every external input, never destroy without confirmation, green before commit, push early and always, least privilege by default.
...
## Rule 3: Distrust every external input
...
## Rule 5: No recoverable history, no autonomy
...
## Rule 10: No anonymous dependencies
...
## Rule 11: Assume no path, no OS, no shell — and no head
...
## Rule 12: Least privilege by default
...
3. **Distrust every external input** — validate at the boundary; parameterize queries and commands, confine paths, never let data become instruction.
4. **Destruction requires a human** — no delete, drop, force-push, or history rewrite without explicit confirmation.
5. **No recoverable history, no autonomy** — agents write only inside git repos with synced remotes; outside, they stop and ask.
...
8. **One purpose per commit** — and per deploy; unrelated and mechanical fixes get filed or split, never bundled.
9. **Fail fast** — bad config or missing backends crash loudly at startup with the key named; never limp along.
10. **No anonymous dependencies** — name, purpose, license, maintenance status, and platform support stated before anything is added.
11. **Assume no path, OS, shell — or head** — cross-platform primitives, headless by default, everything scripted; the platform is an input, not a constant.
12. **Least privilege by default** — narrowest scope that works, no wildcard or shared admin keys, expire and rotate, widen only on a real failure.
13. **The Powell rule** — gather 90% of the information, then decide; below 90% certain, ask the human — never guess, never stall.
