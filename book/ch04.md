...
Two proofs, twenty rules, one discipline — and the order runs by stakes, not by subject. Containment opens, because nothing else matters if the artifact is radioactive: the hooks that prevent the leak lead (Rule 61), and the rotation protocol and the no-copying rule follow (Rules 62 and 63), because those are the moments of maximum irreversibility — the ones where the right move has to be decided before the emergency. Rule 64 is the grade: the 90/95 rubric bar that decides whether anything ships at all, placed ahead of the remaining scan mechanics on purpose. From there the scan gates and the testing structure take their places by how much each one protects, and the complexity budget — small functions, shallow nesting — closes the chapter, exactly as promised above.
...
## Rule 62: Rotate First, Clean History Second
...
The full protocol: stop what you're doing. Surface the finding to the credential's owner verbatim — the SHA, the file, the secret itself, unredacted, because the owner needs to identify exactly which credential to kill (this disclosure to the owner is the one exception to Rule 63's no-copying rule, and it goes to the owner only). Rotate. *Then*, with explicit sign-off — history rewrites are destructive operations under Rule 3 — clean up with `git filter-repo` or BFG, and ask the host to purge its caches. Then proceed to Rule 76, because the incident isn't over until the hooks are smarter.
...
sequenceDiagram
    ...
    D->>D: Fix the hooks that missed it - Rule 76
```
...
## Rule 63: Never Copy a Secret Anywhere
...
## Rule 65: Scan the Whole Artifact
...
So the pre-push scan runs over the full range: everything in `<upstream>..HEAD`, every commit, every version of every file. Gitleaks does this natively against a commit range, and the pre-push hook from Rule 72 is the natural place to wire it in. The cost is seconds; the asymmetry with the alternative is total.

When the range scan does fire on an intermediate commit, the push stops — full stop. And read Rule 62 carefully before you reach for an interactive rebase to scrub the offending commit: if the range was ever pushed anywhere previously, even to a private fork, even briefly, the key is already burned and rotation comes first. The range scan is also why I distrust "I'll squash it away later" as a workflow. Later has a way of arriving after the push.

---

The heaviest containment calls are now made — prevention, rotation, non-propagation, and the two widest scans. The remaining scan mechanics take their places further down, ranked like everything else here by what they protect. Rule 64 already named the bar the second proof answers to; what follows now is its machinery — establishing that what ships actually works, by inspection rather than expectation. The maxim is the same one that drove the scan gates — you get what you inspect — pointed at the software itself.
...
## Rule 70: Leaks Hide in "Harmless" Config
...
## Rule 71: Scan What You Didn't Write
...
## Rule 73: Correctness over speed
...
If you hold the line at 100% (Rule 69), this rule enforces itself — the runner simply fails. Rule 74 exists for the transition period, the legacy repo you're ratcheting upward, the project you inherited at 60%. Whatever today's number is, it is the floor. The ratchet only turns one way, and there is no lever.

## Rule 75: The .gitignore Knows About Keys on Day One
...
## Rule 78: No network in unit tests
...
The escape moves are mechanical: guard clauses that return early and flatten the happy path; extracting the inner block into a named function, which converts a nesting level into a name and a directly testable seam; flipping conditions to fail fast. Each extraction feeds Rule 79's small functions, which feed Rule 69's coverage budget. The testing half of this chapter is one system. This rule is its floor.

### Chapter 4 card

- **61.** Secret-scanning pre-commit hooks (gitleaks + detect-secrets) in every repo, installed *before* the first commit.
- **62.** A secret that ever touched a commit is burned: rotate first, clean history second; pushed objects outlive deletion.
- **63.** Never copy a discovered secret anywhere — not chat, not a scratch file, not "temporarily." Report location, not payload.
- **64.** Keep a rubric and grade the software: 90 is the working bar, polish to 95, publish at 95+.
- **65.** Scan the full deploy artifact — build context, rendered manifests, env bindings — never just the delta.
...
- **70.** Read the staged diff of every config-shaped file before committing; leaks hide best in "harmless" YAML.
- **71.** Scan any file you didn't author for secret-shaped strings before `git add`.
- **72.** Pre-push hooks rescan for secrets and run the tests; the push is the last gate you control.
- **73.** Correctness over speed — the verification delay is budgeted and expected.
- **74.** Coverage going down is stop-and-fix, not "justify it."
- **75.** `.gitignore` covers `.env*`, keys, certs, and credential files from day one — by category, not by incident.
- **76.** After any leak, identify the scan that would have caught it and fix the hooks so it can't recur.
- **77.** Full regression after every feature; report the test count, not just "green."
- **78.** No network in unit tests; fakes, mocks, and fixtures only.
