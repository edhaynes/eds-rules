...
- **Status:** In Progress — 2026-06-14: **pass 1 applied to RULES.md** (Eddie signed off, chose hold-at-100). 10 high-value merges (secret 10→5, versioning 8→6, size 5→3, config 8→7) freed 10 slots; refilled with 10 stronger rules (least-privilege #12, UTC #42, reversible migrations #50, timeouts/retries #75, trace-IDs #76, + the new *Operating an AI fleet* chapter #93–100). Count held at exactly 100; rules.txt regenerated. Eddie expects 2 more passes. **Downstream still stale → regenerate:** quality/grades.csv (re-grade the new 100), the Ep91–140 map (pairings key off old #/scores), README.
