#!/usr/bin/env python3
"""Ep94 brand assets: Bard-owl splash screen + lower-third explainer cards (for
non-technical viewers). Brand: black #101010, Red Hat red #EE0000, Arial, the owl.
Requires Pillow.
"""
import os
...
def splash(title, subtitle):
    ...
    ctr(d, 712, f"EPISODE 94", font(40), RED)
...
# plain-English explainers for non-technical viewers (term, gloss)
GLOSSARY = {
    "axiom":    ("Axiom", "A foundational rule taken as always true — the bedrock the rest are built on."),
    "powell":   ("The 90% Rule", "Gather about 90% of what you need, then decide. Don't stall waiting for certainty."),
    "failfast": ("Fail Fast", "Stop with a clear error the moment something's wrong, instead of limping along broken."),
    "api":      ("API", "The agreed interface one piece of software uses to talk to another. Define it first, then build to it."),
    "fleet":    ("AI Fleet", "A team of AI models working together, each given a task sized to what it can handle."),
    "stack":    ("Podman · UBI · OpenShift", "Red Hat's container stack: Podman runs apps in isolated 'containers'; UBI is the hardened base they're built on; OpenShift runs and scales them."),
}

TITLE = "Guy's Scalpel"
SUBTITLE = "100 rules, cut to a layered core"
